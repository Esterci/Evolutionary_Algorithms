...
Current runners enable logarithmic LR decay and early stopping. See
[training controls](README_training_controls.md) for settings and the distinction
between planned and executed budgets; recorded studies below used older defaults.
...
python -m runners.train_pinn_de_adam \
...
loss evaluations each, use `bash shells/run_recommended_pair.sh`. It uses the
...
`bash shells/run_de32_pair.sh` to reproduce that larger schedule. Its report
...
The initial network's log genes are -log(initial coefficients), and the other individuals sample around
them according to the selected population initialization. `--log-weight-init-bound 4`
sets the half-width around each initial log gene; it does not constrain subsequent evolution.
...
components into one loss. Adaptive weights start from the registered coefficients
(default 1000 per combined term), then evolve independently of those fixed values. At zero log variances, the
...
Use `--no-evolve-loss-weights` for the configured fixed objective
(default `1000*(MSE_initial + MSE_boundary + MSE_PDE)`). Coupled with the Adam runner's
`--no-adaptive`, this matches fixed loss weighting and disables adaptation in
both hybrid stages. Its fixed reference retains the same configured coefficients.
...
python -m utils.plot_pinn_results \
  --run-directory pinn_sim/ADAM_RUN \
  --run-directory pinn_sim/DE_ADAM_RUN
```
...
## Which stage optimizes loss coefficients

`--loss-weight-stage` controls only the Initial, Boundary, and PDE loss
coefficients. Network weights and biases are always optimized by DE and then
Adam in this runner. `pinn` denotes the Adam stage of the hybrid experiment.

| Mode | DE stage | Adam/PINN stage |
| --- | --- | --- |
| `de` | Evolve network and three loss log variances | Train network with selected log variances frozen |
| `pinn` | Evolve network with configured fixed coefficients | Train network and log variances initialized as -log(coefficients) |
| `both` (default) | Evolve network and loss log variances | Continue updating both from the selected individual |
| `none` | Evolve network with configured fixed coefficients | Train network with the same fixed coefficients |

```bash
# From work_4, with the updated framework selected:
python -m runners.train_pinn_de_adam --loss-weight-stage de
python -m runners.train_pinn_de_adam --loss-weight-stage pinn
python -m runners.train_pinn_de_adam --loss-weight-stage both
LOSS_WEIGHT_STAGE=de bash shells/run_de32_pair.sh
```

In `de` mode the frozen log variances remain in the framework loss formula
`sum(exp(-s_i)*MSE_i+s_i)` but are excluded from Adam's optimizer parameters.
Their history is saved and remains constant. The regularization is constant
with respect to the network; the framework raw-loss-sum stopping monitor is
retained, as in the other modes using log variances.

In `pinn` mode, configured coefficients during DE match the initial adaptive
coefficients of Adam. The `none` mode retains those coefficients throughout.
DE has no loss genes in either mode. Defaults and overrides are described in
[shared initial coefficients](README_loss_weight_study.md#shared-initial-loss-coefficients).
Historical studies retain their original recorded coefficients.

Metadata records the selected mode, which stages update coefficients, the Adam
initial log variances, and final coefficients and score computed from Adam's
actual state. The scheduler and early stopping continue to apply to Adam.

Legacy `--evolve-loss-weights` maps to `both`; `--no-evolve-loss-weights` maps
to `none`. Do not combine these aliases with `--loss-weight-stage`.
Tiny smoke tests cover all four modes; no full scientific comparison of the
modes has been performed.
