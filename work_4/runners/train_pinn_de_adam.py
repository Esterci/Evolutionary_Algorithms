...
From the repository root (using an environment with the local Pinn-Torch)::
    ...
    python -m runners.train_pinn_de_adam --epochs 1000 --de-generations 100
...
from methods.loss_weight_config import add_loss_weight_arguments, weights_from_arguments, loss_log_vars
...
def source_metadata():
    ...
    local_sources = [
        BASE_DIRECTORY / name
        for name in (
            "methods/differential_evolution.py",
            "methods/pinn_de_adam.py",
            "runners/train_pinn_de_adam.py",
            "methods/pinn_model.py",
            "methods/loss_weight_config.py",
...
def parse_args(argv=None):
    ...
    parser.add_argument("--loss-weight-stage", choices=["de", "pinn", "both", "none"],
                        help="Stage optimizing loss coefficients; default both. Network training always uses DE then Adam.")
    parser.add_argument(
        "--evolve-loss-weights",
        "--evolve-pde-weights",
    ...
    parser.add_argument(
        ...
        type=float,
        default=4.0,
        help="Log-variance initialization half-width around -log(initial weights); no clipping during DE or Adam",
    )
    parser.add_argument("--hidden-sizes", type=int, nargs="+", default=[32] * 3)
    parser.add_argument("--activation-functions", nargs="+", help="One activation per hidden layer; default Tanh. Names are case-insensitive.")
    add_control_arguments(parser)
    add_loss_weight_arguments(parser)
    ...
    args.initial_loss_weights = weights_from_arguments(args, parser)
    legacy_flags = {'--evolve-loss-weights', '--no-evolve-loss-weights',
                    '--evolve-pde-weights', '--no-evolve-pde-weights'}
    if args.loss_weight_stage is not None and any(a.split('=')[0] in legacy_flags for a in arguments):
        parser.error('Use --loss-weight-stage alone; do not combine it with legacy weight flags')
    if args.loss_weight_stage is None:
        args.loss_weight_stage = 'both' if args.evolve_loss_weights else 'none'
    args.evolve_loss_weights = args.loss_weight_stage in ('de', 'both')
    args.adam_optimize_loss_weights = args.loss_weight_stage in ('pinn', 'both')
    ...
    try:
        args.activation_functions = resolve_activations(args.hidden_sizes, args.activation_functions)
    ...
    control_config(args, parser)
    return args


def main(argv=None):
    args = parse_args(argv)
    controls = control_config(args)
    ...
    model, registered_trainer = build_trainer(
        ...
        initial_loss_weights=args.initial_loss_weights,
    )
    use_loss_log_vars = args.loss_weight_stage != 'none'
    ...
    metadata = dict(
        ...
        activation=args.activation_functions[0] if len(set(args.activation_functions)) == 1 else "mixed",
        activation_functions=args.activation_functions,
        ...
        betas=[0.9, 0.999],
        adaptive_loss=use_loss_log_vars,
        loss_weight_stage=args.loss_weight_stage,
        adam_optimize_loss_weights=args.adam_optimize_loss_weights,
        de_adaptive_loss_weights=args.evolve_loss_weights,
        adaptive_formula='sum(exp(-s_i)*MSE_i + s_i)' if use_loss_log_vars else None,
        loss_weight_names=[loss.name for loss in objective.losses],
        adaptive_weight_dtype=str(problem.dtype) if use_loss_log_vars else None,
        adam_loss_weighting=dict(both='adaptive from selected DE log variances',
                                 de='frozen selected DE log variances',
                                 pinn='adaptive from configured initial coefficients',
                                 none='fixed configured coefficients')[args.loss_weight_stage],
        loss_log_variance_bounds=None,
        loss_log_variance_initialization_box=(
            {loss.name: [float(bounds[0][objective.network_dimension + i]),
                         float(bounds[1][objective.network_dimension + i])]
             for i, loss in enumerate(objective.losses)}
            if args.evolve_loss_weights
            else None
        ),
        configured_initial_loss_weights=args.initial_loss_weights,
        loss_population_initialization="First individual uses configured weights; other log genes use the configured local/uniform sampling around their initial values",
        initial_loss_weights={
            loss.name: weight
            for loss, weight in zip(objective.losses, objective.weights)
        },
        loss_weights=None if use_loss_log_vars else args.initial_loss_weights,
        ...
        fitness_formula=(
            ...
            if args.evolve_loss_weights
            else " + ".join(f"{w:g}*MSE_{loss.name}" for loss, w in zip(objective.losses, objective.reference_weights))
        ),
    ...
    try:
        ...
        adam_initial_log_vars = (
            objective.loss_log_vars if args.evolve_loss_weights else
            loss_log_vars(registered_trainer.lossesW, device=args.device, dtype=problem.dtype) if use_loss_log_vars else None
        )
        metadata['adam_initial_loss_log_vars'] = dict(zip(
            metadata['loss_weight_names'], adam_initial_log_vars.cpu().tolist()
        )) if adam_initial_log_vars is not None else {}
        adam_trainer = make_adam_trainer(
            registered_trainer,
            adam_epochs,
            args.lr,
            args.print_steps,
            loss_log_vars=adam_initial_log_vars,
            optimize_loss_weights=args.adam_optimize_loss_weights,
            training_controls=controls,
        )
        start = time.perf_counter()
        model, history, log_history = train_with_loss_weight_history(adam_trainer)
        metadata["adam_seconds"] = time.perf_counter() - start
        metadata.update(save_training_controls(adam_trainer, controls, run, result.fitness_evaluations))
        ...
        final_log_vars = (adam_trainer.adaptive_weights.log_vars.detach()
                          if use_loss_log_vars else torch.empty(0))
        if not torch.isfinite(final_log_vars).all() or not torch.isfinite(log_history).all():
            raise FloatingPointError('Non-finite final adaptive weights')
        final_weights = (torch.exp(-final_log_vars).cpu().tolist()
                         if use_loss_log_vars else objective.weights)
        final_score = sum(weight * final_losses[loss.name]
                          for weight, loss in zip(final_weights, objective.losses))
        final_score += float(final_log_vars.sum())
        ...
        metadata["final_loss_log_vars"] = dict(zip(
            metadata['loss_weight_names'], final_log_vars.cpu().tolist()))
        ...
        print(f"Plot with: python -m utils.plot_pinn_results --run-directory {run}")
