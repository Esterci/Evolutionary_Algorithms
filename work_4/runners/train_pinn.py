...
Examples (from the work_4 root)::

    python -m runners.train_pinn --epochs 5000
    python -m runners.train_pinn --epochs 2 --hidden-sizes 8 8 --device cpu
    python -m runners.train_pinn --checkpoint pinn_sim/pinn_model_TIMESTAMP.pt
...
from methods.fvm_model_serial import solve_pde
...
def main(argv=None):
    ...
    controls = control_config(args, parser)
    ...
    if args.checkpoint:
        ...
        args.activation_functions = metadata.get("activation_functions")
    ...
    model, trainer = build_trainer(
        ...
        training_controls=controls if not args.checkpoint else None,
    ...
    if args.checkpoint:
        ...
        metadata = dict(
            ...
            optimizer=('external torch.optim.Adam with framework AdaptiveLossWeights in Trainer'
                       if args.adaptive else 'external torch.optim.Adam in FisiocomPINN Trainer'),
            betas=[0.9, 0.999],
            adaptive_loss=args.adaptive,
            initial_loss_weights=args.initial_loss_weights,
            loss_weights=None if args.adaptive else args.initial_loss_weights,
            ...
            adam_loss_weighting='adaptive from configured coefficients' if args.adaptive else 'fixed configured coefficients',
            loss_log_variance_bounds=None,
            initialization="Xavier uniform weights; zero biases; configured hidden activations",
        ...
        metadata.update(save_training_controls(trainer, controls, run))
        ...
        if args.adaptive:
            ...
            metadata.update(
                initial_loss_weights=args.initial_loss_weights,
                initial_loss_log_vars=dict(zip(names, loss_log_vars(
                    args.initial_loss_weights.values(), device=args.device, dtype=problem.dtype).cpu().tolist())),
    ...
    print(
        f'Plot with: python -m utils.plot_pinn_results --run-directory {run}'
    )
