...
from methods.pinn_model import BurgersProblem, NormalizedFisiocomPINN, predict_in_batches
from runners.train_pinn import compare_fvm
from runners.train_pinn_de_adam import physics_validation
...
def evaluate(run, reference, test=False):
    ...
    problem = BurgersProblem(metadata['mesh'], metadata['constants'], dtype)
    model = NormalizedFisiocomPINN(problem, metadata['architecture'], metadata.get('activation_functions')).to(device=device, dtype=dtype)
    model.load_state_dict(torch.load(run / 'pinn_model.pt', map_location=device, weights_only=True))
    model.eval()
    ...
    for configuration in configurations:
        for seed in seeds:
            for method in ('pinn', 'pinn-de'):
                ...
                command = [sys.executable, '-u', str(BASE / 'runners' / script), '--config-directory', str(root / 'config'),
                ...
                if method == 'pinn-de':
                    command += ['--loss-weight-stage', de.get('loss_weight_stage', 'both'),
                                '--de-generations', str(de['generations']), '--population-size', str(de['population_size']),
                ...
                print(f'START {label}: up to {budget} training loss evaluations', flush=True)
