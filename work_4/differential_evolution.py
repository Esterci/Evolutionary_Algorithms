...
class DEConfig:
    """Configuration for synchronous generations and finite initialization.

    ``generations`` excludes initialization: the evaluation budget is
    ``population_size * (generations + 1)``. Bounds apply to every genome
    coordinate unless clipping is disabled with ``bound_mask``. Supplied
    per-coordinate bounds define each coordinate's initialization box. The first
...
def differential_evolution(
    ...
    *,
    ...
    ``bound_mask`` is an optional boolean [D] tensor on the genome's device.
    A false entry allows that coordinate to evolve outside its initialization
    interval, while true entries retain clipping. Initialization always uses
    the finite intervals, and the initial vector must be inside them. Omitting
    the mask clips every coordinate, preserving the default bounded search.
    ...
    Every candidate is passed to the objective, including nonfinite genomes
    that may arise from overflow in unbounded mutation. The objective must
    handle those inputs without raising (for example, by returning +inf).
    Nonfinite genomes or scalar values count as failed evaluations and receive
    +inf; programming exceptions propagate.
    ...
    deviation divided by that coordinate's initialization-interval width,
    including coordinates that subsequently evolve without clipping. Generation-zero
    ...
    if bound_mask is not None:
        if not isinstance(bound_mask, Tensor):
            raise ValueError("bound_mask must be a boolean [D] tensor")
        if bound_mask.shape != initial_vector.shape:
            raise ValueError("bound_mask must have exactly the initial_vector shape")
        if bound_mask.dtype != torch.bool or bound_mask.device != device:
            raise ValueError("bound_mask must use bool dtype and initial_vector device")
        bound_mask = bound_mask.detach().clone()
    ...
    def evaluate(vector: Tensor) -> float:
        ...
        value = float(value)
        if (
            not torch.isfinite(vector).all()
            or not math.isfinite(value)
            or abs(value) > torch.finfo(dtype).max
        ):
        ...
        history.append(
            {
                ...
                "best_index": int(fitness.argmin().item()),
    ...
    for generation in range(1, config.generations + 1):
        ...
        for index in range(population_size):
            ...
            clipped_mutant = torch.maximum(torch.minimum(mutant, upper), lower)

            mutant = (
                clipped_mutant
                if bound_mask is None
                else torch.where(bound_mask, clipped_mutant, mutant)
            )
