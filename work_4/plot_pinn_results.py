r"""Plot saved Adam/DE-to-Adam PINNs, FVM fields, errors and training histories.
    ...
    python work_4/plot_pinn_results.py --run-directory work_4/pinn_sim/ADAM_RUN \
        --run-directory work_4/pinn_sim/DE_ADAM_RUN --frames 60 --fps 12
...
def comparison_panel(arrays, predictions=None):
    """Use shared field and error scales across every method and time level."""
    predictions = predictions or {"PINN": arrays}
    columns = 1 + 2 * len(predictions)
    figure, axes = plt.subplots(
        2, columns, figsize=(4 * columns, 7), constrained_layout=True
    )
    images, fields = [], []
    for row, variable in enumerate(("u", "v")):
