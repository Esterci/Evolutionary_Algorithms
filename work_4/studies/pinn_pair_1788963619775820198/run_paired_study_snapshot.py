    ...
    for configuration in configurations:
        for seed in seeds:
            for method in ('pinn', 'pinn-de'):
                ...
                command = [sys.executable, str(BASE / script), '--config-directory', str(root / 'config'),
                ...
                if method == 'pinn-de':
                    command += ['--de-generations', str(de['generations']), '--population-size', str(de['population_size']),
                                '--initial-spread', str(de['initial_spread']), '--de-seed', str(manifest['de_seed']),
                ...
                record = dict(label=label, phase=phase, configuration=configuration, seed=seed, method=method,
                              budget=budget, command=command, returncode=completed.returncode,
