...
from pinn_model import Trainer
...
class PINNFitness:
    ...
    def bounds(self, lower, upper, log_bound=4.0):
        """Create network bounds and a finite symmetric loss-gene initial box."""
        if (
        ...
        if self.evolve_loss_weights:
            ...
            lower_vector[self.network_dimension :] = -log_bound
            upper_vector[self.network_dimension :] = log_bound
        if (
