...
def run_schedule(record):
    ...
    return dict(
        ...
        adam_updates=metadata.get('adam_epochs') if hybrid else metadata.get('epochs'),
...
def analyze(root, select=False):
    ...
    for phase, records in [('screen', screen), ('confirm', confirm)]:
        ...
        for record in records:
            ...
            row = dict(configuration=record['configuration']['name'], method=record['method'],
                       ...
                       evaluations=record['budget'], training_seconds=values.get('training_seconds'),
    ...
    if screen:
        ...
        fig.suptitle('Exploratory screening: one common seed, equal loss-evaluation budgets')
